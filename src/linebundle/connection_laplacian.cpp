#include "connection_laplacian.h"

namespace minimalmatching
{
  namespace BundleFemDetails
  {
    /*
    ...
    s11(const double t)
    {
      ...
      return Vector2(csevl(t, s11r, ns11r), csevl(t, s11i, ns11i));
    }
    ...
    s12(const double t)
    {
      ...
      return Vector2(csevl(t, s12r, ns12r), csevl(t, s12i, ns12i));
    }
    ...
    m12(const double t)
    {
      ...
      return Vector2(csevl(t, m12r, nm12r), csevl(t, m12i, nm12i));
    }
    ...
    {
      const double s2(s * s);
      const Vector2 is(0, s), is3(0, s * s2);
      const double s4(s2 * s2);
      return
      ((Vector2(3 * gii + 4 * gij + 3 * gjj, 0) + is * (gii + gij + gjj) - is3 * gij / 6
          + Vector2(cos(s), sin(s)) * (-Vector2(3 * gii + 4 * gij + 3 * gjj, 0)
            + is * (2 * gii + 3 * gij + 2 * gjj)
            + s2 * Vector2(gii + 2 * gij + gjj, 0) / 2)) / s4
        + Vector2(gii - 2 * gij + gjj, 0) / 24 - is * (gii - 2 * gij + gjj) / 60);
    }
    ...
    {
      return {.25 * ((gjj - 2 * gjk + gkk) + s * s * (gjj + gjk + gkk) / 90.), 0};
    }

    Vector2
    MassII(void) { return {1 / 6., 0}; }
    ...
    MassIJDirect(const double s)
    {
      ...
      const double s4 = s2 * s2;
      const Vector2 is(0, s), is3(0, s * s2);
      return (6 * Vector2(cos(s), sin(s)) - Vector2(6, 0) - 6 * is + 3 * Vector2(s2, 0) + is3) / (3 * s4);
    }
    ...
    Vector2 curvatureFactor(double c, double cutoff = 1e-8)
    {
      double c2 = c * c;
      if (abs(c) < cutoff)
      {
        // Return taylor expansion for values near
        // removable singularity at 0
        double re = 1 / 12. + c2 * (-1. / 360. + c2 / 20160.);
        double im = c * (1. / 60. + c2 * (-1. / 2520. + c2 / 181440.));
        return {re, im};
      }
      else
      {
        double c3 = c2 * c, c4 = c2 * c2;
        return Vector2{6 * cos(c) - 6 + 3 * c2, 6 * sin(c) - 6 * c + c3} /
          (3 * c4);
      }
    }

    Vector2 f1(double s, double cutoff = 1e-8)
    {
      double s2 = s * s;
      if (abs(s) < cutoff)
      {
        double re = s2 * (1. / 120. + s2 * (-1. / 2688. + s2 / 129600));
        double im = s * (-1. / 24. + s2 * (1. / 504. - s2 / 17280));
        return {re, im};
      }
      else
      {
        double s4 = s2 * s2, s5 = s2 * s2 * s;
        return (Vector2{3. + s4 / 24., s - s5 / 60.} +
            Vector2{-3. + s2 / 2., 2. * s} * Vector2::fromAngle(s)) /
          s4;
      }
    }

    Vector2 f2(double s, double cutoff = 1e-8)
    {
      double s2 = s * s;
      if (abs(s) < cutoff)
      {
        double re =
          -1. / 4. + s2 * (1. / 45. + s2 * (-1. / 1120 + s2 / 56700.));
        double im = s * (-1. / 24. + s2 * (5. / 1008. - s2 * 7. / 51840));
        return {re, im};
      }
      else
      {
        double s3 = s * s2, s4 = s2 * s2, s5 = s2 * s2 * s;
        return (Vector2{4. - s4 / 12., s - s3 / 6. + s5 / 30.} +
            Vector2{-4. + s2, 3 * s} * Vector2::fromAngle(s)) /
          s4;
      }
    }
  } // namespace BundleFemDetails
  ...
  complexBundleMassMatrix(ManifoldSurfaceMesh& mesh,
                          ...
                          const FaceData<double>& curvature, bool transpose)
  {
    ...
    {
      double a = geom.faceAreas[he.face()];

      size_t i = geom.vertexIndices[he.next().tipVertex()];
      size_t j = geom.vertexIndices[he.tailVertex()];
      size_t k = geom.vertexIndices[he.tipVertex()];

      if (transpose) std::swap(j, k);
      Vector2 rjk = connection[he].unit();

      Vector2 selfWeight = a * BundleFemDetails::MassII();
      Vector2 edgeWeight = a * rjk.conj() * BundleFemDetails::MassIJ(curvature[he.face()]);

      T.emplace_back(i, i, selfWeight);
      T.emplace_back(j, k, edgeWeight);
      T.emplace_back(k, j, edgeWeight.conj());
    }
    geom.unrequireFaceAreas();
    geom.unrequireVertexIndices();
    SparseMatrix<std::complex<double>> massMatrix(mesh.nVertices(),
                                                  mesh.nVertices());
    massMatrix.setFromTriplets(T.begin(), T.end());
    return massMatrix;
  }
  ...
  complexBundleLaplacian(ManifoldSurfaceMesh& mesh,
                         IntrinsicGeometryInterface& geom,
                         const HalfedgeData<Vector2>& connection,
                         const FaceData<double>& curvature, bool transpose, double s)
  {
    geom.requireVertexIndices();
    geom.requireFaceAreas();
    geom.requireEdgeLengths();
    std::vector<Eigen::Triplet<std::complex<double>>> T;
    for (Halfedge jk : mesh.halfedges())
    {
      Halfedge ki = jk.next(), ij = jk.next().next();

      double a = geom.faceAreas[jk.face()];
      double lij2 = geom.edgeLengths[ij.edge()] * geom.edgeLengths[ij.edge()];
      double ljk2 = geom.edgeLengths[jk.edge()] * geom.edgeLengths[jk.edge()];
      double lki2 = geom.edgeLengths[ki.edge()] * geom.edgeLengths[ki.edge()];

      // dot product of edge vectors: lij^2 + lki^2 - 2 <pi, Pike> = ljk^2
      double pijpik = ((lij2 + lki2 - ljk2) / 2.);

      size_t i = geom.vertexIndices[jk.next().tipVertex()];
      size_t j = geom.vertexIndices[jk.tailVertex()];
      size_t k = geom.vertexIndices[jk.tipVertex()];
      if (transpose) std::swap(j, k);
      Vector2 rjk = connection[jk].unit();

      double W = curvature[jk.face()];
      Vector2 selfWeight = BundleFemDetails::DirichletII(W, lij2, pijpik, lki2) / a;
      Vector2 edgeWeight = rjk.conj() * BundleFemDetails::DirichletIJ(W, lij2, pijpik, lki2) / a;
    ...
    geom.unrequireFaceAreas();
    geom.unrequireVertexIndices();
    SparseMatrix<std::complex<double>> L(mesh.nVertices(), mesh.nVertices());
    L.setFromTriplets(T.begin(), T.end());
    return L;
  }

  SparseMatrix<std::complex<double>> complexBundleLaplacianDEC(ManifoldSurfaceMesh& mesh,
                                                               IntrinsicGeometryInterface& geom,
                                                               const HalfedgeData<Vector2>& connection,
                                                               const FaceData<double>& curvature, bool transpose)
  {
    ...
    for (Halfedge he : mesh.halfedges())
    {
      ...
      size_t iTip = geom.vertexIndices[he.next().vertex()];

      double weight = std::max(geom.edgeCotanWeights[he.edge()], 1e-2);
      if (std::isnan(weight))
        weight = 1;
      Vector2 rot = connection[he.twin()];
      T.emplace_back(iTail, iTail, std::complex<double>(weight, 0));
      T.emplace_back(iTail, iTip, -weight * rot);
    }
  ...
  SparseMatrix<std::complex<double>> complexBundleMassMatrixDEC(ManifoldSurfaceMesh& mesh,
                                                                IntrinsicGeometryInterface& geom,
                                                                const HalfedgeData<Vector2>& connection,
                                                                const FaceData<double>& curvature, bool transpose)
  {
    geom.requireVertexDualAreas();
    return SparseMatrix<double>(geom.vertexDualAreas.raw().asDiagonal()).cast<std::complex<double>>();
  }
}
